...
from solving_puzzle.piece import Piece
...
def flatten_image(image, piece_size, indexed=False):
    ...
    flattened into a list. Each list element is PIECE_SIZE x PIECE_SIZE x 3.

    :params image:      Input image.
    :params piece_size: Size of single square piece. Each piece is PIECE_SIZE x PIECE_SIZE.
    :params indexed:    If True, a list of Pieces with IDs will be returned. Otherwise, just a plain list of ndarray pieces.
    ...
    for y in range(rows):
        for x in range(columns):
            ...
            pieces.append(Piece(piece, index=(y * columns + x)))

    if indexed:
        return pieces, rows, columns
    else:
        return [piece.image for piece in pieces], rows, columns
